...
[[ -z $SHIELDS ]] && SHIELDS="$(grep '^[[:space:]]*shield:' "$HOST_CONFIG_DIR/build.yaml" | sed 's/^.*: *//')"
...
if [[ -f config/combos.dtsi ]]; then # update maximum combos per key
  count=$(
...
compile_shield() {
  SHIELD="${1}"
  SAFE_SHIELD=$(echo "$SHIELD" | tr ' ' '_')
  BUILD_DIR="${SAFE_SHIELD}_$SUFFIX"
  LOGFILE="$LOG_DIR/zmk_build_${SAFE_SHIELD}.log"
  [[ $MULTITHREAD = "true" ]] || echo -en "\n$(tput setaf 2)Building $1... $(tput sgr0)"
  [[ $MULTITHREAD = "true" ]] && echo -e "$(tput setaf 2)Building $1... $(tput sgr0)"
  $DOCKER_PREFIX west build -d "build/$BUILD_DIR" -b "nice_nano_v2" "$DOCKER_ZMK_DIR/app" -- -DZMK_CONFIG="$CONFIG_DIR" -DSHIELD="${SHIELD}" -Wno-dev >"$LOGFILE" 2>&1
  if [[ $? -eq 0 ]]; then
    ...
    OUTPUT="$OUTPUT_DIR/$SAFE_SHIELD-zmk.$TYPE"
...
if [[ $MULTITHREAD = "true" ]]; then
  ...
  echo -e "$SHIELDS" | while IFS= read -r shield; do
    compile_shield "$shield"
  done
fi
